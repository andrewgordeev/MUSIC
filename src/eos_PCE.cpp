...
EOS_PCE::EOS_PCE() {  // This version of the code uses a pure glue equation of state. Code to interpolate two equations of state for partial chemical equilibrium is removed.
    set_EOS_id(18);   
...
double EOS_PCE::p_e_func(double e, double rhob) const {
...
double EOS_PCE::get_temperature(double e, double rhob) const {
    double T_gluon = interpolate1D(e, 0, temperature_tb);
    return(std::max(1e-15, T_gluon));
}


//! This function returns the local pressure in [1/fm^4]
//! the input local energy density [1/fm^4], rhob [1/fm^3]
double EOS_PCE::get_pressure(double e, double rhob) const {
    double f_gluon = interpolate1D(e, 0, pressure_tb); // 1/fm^4
    return(std::max(1e-15, f_gluon));
}


double EOS_PCE::get_s2e(double s, double rhob) const {
    double e = get_s2e_finite_rhob(s, 0.0);
    return(e);
}

double EOS_PCE::get_T2e(double T, double rhob) const {
